...
class StepInfo:
    ...
    grad_norm: float
    mu_k: float
    prob: float = 0.0          # softmax P(target_class | γₖ); filled by callers
...
BASELINE_MODES = ("black", "white", "noise", "blur", "mean_corners")
...
def make_baseline(
    ...
    """
    Construct a baseline tensor (same shape/device/dtype as x_normalized).

    The baseline is first built in the raw [0,1] pixel space, then normalised
    with the given mean/std so the returned tensor is directly consumable by
    the backbone (which expects ImageNet-normalised input).

    mode:
        black         — pixel (0,0,0) → normalised = -mean/std
        white         — pixel (1,1,1)
        noise         — Gaussian N(0.5, 0.25) in [0,1], seeded for repro
        blur          — Gaussian blur of the input (σ=20, kernel 51)
        mean_corners  — mean of 3×3 patches at the 4 corners of the input,
                        broadcast to a constant-colour baseline
    """
    ...
    # Recover the input in raw [0,1] pixel space (for blur / mean_corners).
    x_raw = (x_normalized * std_t + mean_t).clamp(0.0, 1.0)

    if mode == "black":
        b_raw = torch.zeros_like(x_raw)
    elif mode == "white":
        b_raw = torch.ones_like(x_raw)
    elif mode == "noise":
        g = torch.Generator(device="cpu").manual_seed(int(seed))
        b_raw = (torch.randn(x_raw.shape, generator=g) * 0.25 + 0.5).to(device=device, dtype=dtype)
        b_raw = b_raw.clamp(0.0, 1.0)
    elif mode == "blur":
        ksize = 51
        sigma = 20.0
        ...
        k1d = k1d / k1d.sum()
        k2d = (k1d[:, None] * k1d[None, :]).view(1, 1, ksize, ksize).expand(C, 1, ksize, ksize)
        pad = ksize // 2
        x_pad = F.pad(x_raw, (pad, pad, pad, pad), mode="reflect")
        b_raw = F.conv2d(x_pad, k2d, groups=C)
    elif mode == "mean_corners":
        patches = torch.cat([
            x_raw[:, :, :3,  :3 ].reshape(B, C, -1),
            x_raw[:, :, :3,  -3:].reshape(B, C, -1),
            x_raw[:, :, -3:, :3 ].reshape(B, C, -1),
            x_raw[:, :, -3:, -3:].reshape(B, C, -1),
        ], dim=-1)  # (B, C, 36)
        mean_val = patches.mean(dim=-1).view(B, C, 1, 1)
        b_raw = mean_val.expand(B, C, H, W).contiguous()

    return (b_raw - mean_t) / std_t
...
def load_image_batch(backbone, device, n, min_conf=0.70, image_dir=None, model_name=None):
    """Load n valid images in one pass, no repeats.

    If image_dir is provided, load from that directory (no shuffle, no skip).
    Otherwise fall back to benchmark_50 with shuffle + confidence filter.
    """
    ...
    if model_name == 'inception_v3':
        tf = T.Compose([
            T.Resize(342), T.CenterCrop(299), T.ToTensor(),
    ...
    if image_dir is not None:
        ...
        search_dirs = ["./benchmark_50", "../benchmark_50"]
        use_shuffle = True
...
def load_image(backbone: nn.Module, device: torch.device, min_conf: float = 0.70, skip: int = 0):
    ...
    # Try local images
    for sample_dir in ["./benchmark_50", "../benchmark_50"]:
        if not os.path.isdir(sample_dir):
            continue
        try:
            from PIL import Image
